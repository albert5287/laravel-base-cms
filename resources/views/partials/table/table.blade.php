...
<div class="box">
    <div class="box-body">
        ...
        {{--<div> {!! str_replace('/?', '?', $search === "" ? $elements->appends(['order' => $order, 'sort' => $sort])->render() : $elements->appends(['order' => $order, 'sort' => $sort, 'search' => $search])->render()) !!} </div>--}}{{--<div> {!! str_replace('/?', '?', $search === "" ? $elements->appends(['order' => $order, 'sort' => $sort])->render() : $elements->appends(['order' => $order, 'sort' => $sort, 'search' => $search])->render()) !!} </div>--}}
    </div><!-- /.box-body -->
</div><!-- /.box -->
@include('partials.table.addButtonTable')
